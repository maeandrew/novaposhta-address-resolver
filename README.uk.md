...
Застосунок може підключити готовий SDK Нової пошти або власний HTTP-клієнт, а
AI-провайдер додати без прив’язки core до конкретного vendor SDK.
...
Вбудований parser розпізнає українські та поширені російські форми `м.`,
`відд.`, `відділення`, `поштомат`, `пункт`, `№`, `НП`, а також номер словами,
наприклад `двісті вісімдесят п’ять`. Matching можна налаштувати:
...
## AI рекомендований для production-ввідних даних

Для вільних production-повідомлень AI варто вважати основним шаром якості. Він
краще обробляє одруки, змішані мови, пропущені назви полів і незвичний формат,
тоді як deterministic pipeline залишається безпечним degraded mode для
тимчасової недоступності provider-а, обмежень приватності, offline-задач і
тестів. Тому core не нав’язує AI SDK або API key.

Контракти AI та fake provider уже є в ядрі. AI може ранжувати лише кандидатів,
отриманих від provider-а, але не може створювати чи зберігати reference
населеного пункту або відділення. Якщо доступно кілька provider-ів, налаштуйте
fallback chain.
